<template>
    <fv-filter-decorator v-if="brokerness" title="Trading Instruments">
        <div>
            <b-row>
                <b-col cols="9">
                    ...
                    <b-form-group>
                        <b-form-checkbox-group
                                id="checkbox-group-trading-instruments"
                                class="filter-content option-filter"
                                :value="selected"
                                v-on:input="onChange($event)"
                                :options="options.slice(0, limit).map(option=> ({text: option.name, value: option.id}))"
                                name="flavour-1"
                                stacked
                        />
                    </b-form-group>
